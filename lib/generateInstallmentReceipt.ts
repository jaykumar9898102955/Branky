...
export async function generateInstallmentReceipt(
  ...
  const remaining = plan.totalRemaining
  ...
  <div style="max-width:720px;margin:0 auto;padding:24px 20px;">
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:0;">
      <tr>
        ...
        <td style="padding:18px 24px;background:#1D5CE3;text-align:right;vertical-align:middle;border-radius:0 12px 0 0;">
          <div style="color:#FF931E;font-size:20px;font-weight:800;letter-spacing:1px;">FEE RECEIPT</div>
      ...
      <tr>
        <td style="width:50%;padding:16px 20px;border-right:1px solid #e2e8f0;vertical-align:top;">
          ...
          <div style="margin-top:6px;display:flex;align-items:center;gap:6px;">
            <span style="font-size:10px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.05em;">Parent:</span>
            <span style="font-size:14px;font-weight:700;color:#1D5CE3;">${student.parentName || '—'}</span>
          </div>
          <div style="margin-top:5px;font-size:12px;color:#64748b;">&#128222; ${student.phone}</div>
          ${student.city ? `<div style="font-size:12px;color:#64748b;margin-top:2px;">&#127968; ${student.city}</div>` : ''}
        </td>
        <td style="width:50%;padding:16px 20px;vertical-align:top;background:#f8fafc;">
          ...
          <div style="margin-top:8px;display:flex;flex-direction:column;gap:4px;">
            ...
            ${plan.discount > 0 ? `<div style="font-size:12px;color:#065f46;font-weight:700;">Discount applied: &#8377;${Number(plan.discount).toLocaleString('en-IN')}</div>` : ''}
          </div>
        </td>
      </tr>
    </table>
    ...
    <table style="width:100%;border-collapse:collapse;border:1.5px solid #e2e8f0;margin-bottom:20px;">
      <thead>
        <tr style="background:#1D5CE3;">
          ...
          <th style="padding:10px 14px;text-align:center;color:#fff;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;">Due Date</th>
      ...
      <tbody>
        <tr style="background:#fff7ed;">
          <td style="padding:14px;font-weight:700;color:#0d0d0d;">
            ...
            ${inst.notes ? `<div style="font-size:11px;color:#64748b;margin-top:4px;font-style:italic;">${inst.notes}</div>` : ''}
          </td>
          <td style="padding:14px;text-align:center;font-weight:700;color:#1D5CE3;">${inst.installmentNumber} / ${plan.durationMonths}</td>
          <td style="padding:14px;text-align:center;color:#64748b;">${fmtDate(inst.dueDate)}</td>
          <td style="padding:14px;text-align:center;font-weight:700;color:#065f46;">${fmtDate(inst.paidDate)}</td>
          <td style="padding:14px;text-align:right;font-size:16px;font-weight:800;color:#065f46;">${fmt(inst.paidAmount > 0 ? inst.paidAmount : inst.amount)}</td>
        </tr>
      </tbody>
    </table>
    ...
    <table style="width:100%;border-collapse:collapse;border:1.5px solid #e2e8f0;margin-bottom:24px;">
      ...
      <tr>
        ...
        <td style="padding:10px 16px;font-weight:600;color:#64748b;width:25%;border-bottom:1px solid #f1f5f9;">Monthly Amount</td>
        <td style="padding:10px 16px;font-weight:700;color:#0d0d0d;width:25%;border-bottom:1px solid #f1f5f9;">${fmt(plan.monthlyAmount)}</td>
      </tr>
      ${plan.discount > 0 ? `
      <tr>
      ...
      <tr>
        <td style="padding:10px 16px;font-weight:600;color:#64748b;border-bottom:1px solid #f1f5f9;">Months Paid</td>
        <td style="padding:10px 16px;font-weight:700;color:#0d0d0d;border-bottom:1px solid #f1f5f9;">${plan.paidCount} / ${plan.durationMonths}</td>
        <td style="padding:10px 16px;font-weight:600;color:#64748b;border-bottom:1px solid #f1f5f9;"></td>
        <td style="padding:10px 16px;border-bottom:1px solid #f1f5f9;"></td>
      </tr>
      <tr style="background:#eff6ff;">
        <td style="padding:12px 16px;font-weight:800;color:#0d0d0d;font-size:14px;">Total Paid</td>
        <td style="padding:12px 16px;font-weight:800;color:#065f46;font-size:17px;">${fmt(paidSoFar)}</td>
        <td style="padding:12px 16px;font-weight:800;color:#0d0d0d;font-size:14px;">${remaining > 0 ? 'Balance Due' : 'Fully Paid ✓'}</td>
        <td style="padding:12px 16px;font-weight:800;font-size:17px;color:${remaining > 0 ? '#dc2626' : '#065f46'};">${remaining > 0 ? fmt(remaining) : '&#10003; Nil'}</td>
      </tr>
    </table>
